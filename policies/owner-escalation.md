...
Do not ask the Owner merely to acknowledge normal progress, green CI, routine completion, an autonomously repairable bug, or a stale historical prerequisite.
...
Every owner escalation must bind to the exact current owner-action/blocker identity and current evidence. Never reuse a stale blocker merely because a previous report or phase document mentioned it. Newer owner feedback/evidence may resolve, supersede or narrow the dependency.
...
7. only that Worker-generated receipt, or an explicit append-only historical reconciliation receipt described below, is authoritative. Codex/LLM prose, a child-process exit code, `Completion email sent...`, app-server `smtp_success` diagnostics, or a similar marker inside the Codex response is not by itself authoritative for the Bridge control plane;
...
Runs that predate the Worker-owned gateway may be reconciled only from concrete historical evidence plus an explicit Owner observation or equivalent authoritative evidence. Store such evidence append-only under `projects/<project-id>/notification-receipts/`. It must use `evidence_type=reconciled_owner_observation` (or another explicitly documented reconciliation type), reference the original command/report, state why native evidence was unavailable, and must never pretend that a native receipt existed at send time. A reconciliation receipt suppresses duplicate resend for the same exact blocker but does not rewrite the historical Report.
...
- `MISSING/REMEDIATION_REQUIRED` — current blocker still requires notification and evidence proves the historical path skipped sending;
...
Do not label the last two cases `blocked-owner` or say the Owner still needs to perform the old action.
...
The complementary future hardening should also cover the **resume side**: exact owner-action thread identity, latest-event ordering, verified/current resolution evidence, stale root-event rejection, idempotent resume, and fresh canonical CAS. A future gateway must never turn an old `AWAITING_OWNER` root event back into a live Owner ask after a newer completion event exists.
